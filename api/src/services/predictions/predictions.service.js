...
module.exports = function (app) {
  class Prediction {
    create(data) {
      ...
      return pythonExec(gradcamSingleRunPath, {
        args: [
          "--sag_path=" + sag_path,
        ...
        .then((result) => {
          ...
          const sagittalPreProcessed = fs.readFileSync(
            UPLOAD_PATH + saggital + "_preprocessed.jpg",
            { encoding: "base64" }
          );
          const sagittalGradCamId = `${saggital}_Cam_On_Image_inferno.png`;
          const sagittalGradCam = fs.readFileSync(
            UPLOAD_PATH + sagittalGradCamId,
            { encoding: "base64" }
          );
          const transversePreProcessed = fs.readFileSync(
            UPLOAD_PATH + transverse + "_preprocessed.jpg",
            { encoding: "base64" }
          );
          const transverseGradCamId = `${transverse}_Cam_On_Image_inferno.png`;
          const transverseGradCam = fs.readFileSync(
            UPLOAD_PATH + transverseGradCamId,
            { encoding: "base64" }
          );
